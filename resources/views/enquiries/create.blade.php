<x-app-layout>
    <div class="mb-4">
        ...
        <h2 class="fw-bold mb-1">Register New Enquiry</h2>
        <p class="text-secondary small mb-0">Manually capture a student visit, registration, or initial lead inquiry.</p>
    </div>
        ...
        <form action="{{ route('enquiries.store') }}" method="POST">
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-md-6 col-lg-4">
                    ...
                    <input type="text" name="student_name" class="form-control" value="{{ old('student_name') }}" required placeholder="e.g. John Doe">
                </div>

                <div class="col-12 col-md-6 col-lg-4">
                    <label class="form-label small fw-semibold">Mobile Number *</label>
                    <input type="text" name="phone_number" class="form-control" value="{{ old('phone_number') }}" required placeholder="10-digit mobile phone">
                    <div class="form-text text-muted" style="font-size: 0.7rem;">Prevents duplicate records automatically.</div>
                </div>

                <div class="col-12 col-md-6 col-lg-4">
                    <label class="form-label small fw-semibold">WhatsApp Number</label>
                    <input type="text" name="whatsapp_number" class="form-control" value="{{ old('whatsapp_number') }}" placeholder="WhatsApp contact">
                </div>

                <div class="col-12 col-md-6 col-lg-4">
                    <label class="form-label small fw-semibold">Parent Phone</label>
                    <input type="text" name="parent_phone" class="form-control" value="{{ old('parent_phone') }}" placeholder="Emergency parent number">
                </div>
                ...
                <div class="col-12 col-md-6 col-lg-4">
                    ...
                    <input type="text" name="qualification" class="form-control" value="{{ old('qualification') }}" placeholder="Degree / Diploma">
                </div>
                ...
                <div class="col-12 col-md-6 col-lg-4">
                    ...
                    <input type="text" name="place" class="form-control" value="{{ old('place') }}" placeholder="City/Village">
                </div>
                ...
                <hr class="text-secondary opacity-25 my-4 col-12">
                <h5 class="fw-bold text-primary mb-1 col-12">3. Registration & Course Assignment</h5>
                ...
                <div class="col-12 col-md-6 col-lg-4">
                    ...
                    <select name="assigned_employee_id" class="form-select">
                        <option value="">Assigned to Self</option>
                        @foreach($employees as $emp)
                            <option value="{{ $emp->id }}" {{ old('assigned_employee_id') == $emp->id ? 'selected' : '' }}>{{ $emp->name }} ({{ $emp->roles->first()?->name }})</option>
                        @endforeach
                    </select>
                </div>
                @endrole
                ...
                <div class="col-12 col-md-6 col-lg-4">
                    ...
                    <select name="current_status" class="form-select" required>
                        <option value="New" {{ old('current_status') === 'New' ? 'selected' : '' }}>New Enquiry</option>
                        <option value="Walk-in" {{ old('current_status') === 'Walk-in' ? 'selected' : '' }}>Walk-in Visit</option>
                        <option value="Registered" {{ old('current_status') === 'Registered' ? 'selected' : '' }}>Registered</option>
                        <option value="Admitted" {{ old('current_status') === 'Admitted' ? 'selected' : '' }}>Admitted</option>
                        <option value="Full Payment" {{ old('current_status') === 'Full Payment' ? 'selected' : '' }}>Full Payment</option>
                    </select>
                </div>

                <div class="col-12">
                    <label class="form-label small fw-semibold">Discussion Remarks / Internal Notes</label>
                    <textarea name="remarks" class="form-control" rows="4" placeholder="Enter conversation points, preferences, follow-up callbacks here...">{{ old('remarks') }}</textarea>
                </div>

                <div class="col-12 d-flex gap-2 justify-content-end mt-4">
                    <a href="{{ route('enquiries.index') }}" class="btn btn-outline-secondary btn-rounded px-4 py-2 small fw-semibold">Cancel</a>
                    <button type="submit" class="btn btn-primary btn-rounded px-4 py-2 small fw-semibold"><i class="bi bi-save me-1"></i> Register Student</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            if (initialScaleId) {
                loadDistricts(initialScaleId, "{{ old('district_id') }}");
            }
